...
Options:
  --init-project      Also initialize docs/ and .coord/ template directories
                      in the current working directory (markdown state)
  --init-project-v2   Initialize docs/ and .coord/ with structured JSON templates
                      and copy schemas/ + bin/validate-state.sh to the project
...
Examples:
  ...
  # Install agents AND initialize v2 JSON state in current directory:
...
for agent in coordinator coordinator-experimental coordinator-experimental-v2 briefer planner worker worker-experimental reviewer ui-tester ux-tester system-tester scribe scribe-v2 intent-validator; do
...
# ─── Initialize v2 project (optional) ───────────────────────────────────────

if [ "$INIT_PROJECT_V2" = true ]; then
  echo ""
  echo "Initializing v2 project templates (structured JSON) in: $(pwd)"

  TEMPLATES_DIR="$SCRIPT_DIR/templates-v2"

  mkdir -p docs/context docs/plans .coord

  for f in current-intent.json repo-practices.json known-issues.json command-intent.json; do
    dst="docs/context/$f"
    if [ -f "$dst" ]; then
      echo "  Skipping (exists): $dst"
    else
  ...
  for f in active-plan.json execution-brief.json test-spec.json; do
    dst="docs/plans/$f"
    if [ -f "$dst" ]; then
      echo "  Skipping (exists): $dst"
    else
  ...
  for f in task-ledger.json learning-inbox.jsonl context-packet.json; do
    dst=".coord/$f"
    if [ -f "$dst" ]; then
      echo "  Skipping (exists): $dst"
    else
  ...
  if [ ! -d "schemas" ]; then
    ...
    echo "  Skipping (exists): schemas/"
  fi

  # Copy validator
  mkdir -p bin
  if [ ! -f "bin/validate-state.sh" ]; then
    cp "$SCRIPT_DIR/bin/validate-state.sh" bin/validate-state.sh
    chmod +x bin/validate-state.sh
    echo "  Created: bin/validate-state.sh"
  else
    echo "  Skipping (exists): bin/validate-state.sh"
  fi

  echo ""
  echo "V2 project initialized. Validate state with: bin/validate-state.sh --all"
fi
